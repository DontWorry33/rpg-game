...
int main()
{
	...
	while (App.IsOpened())
	{
		sf::Vector2f MousePos = App.ConvertCoords(App.GetInput().GetMouseX(), App.GetInput().GetMouseY(), &locked);
		App.Clear();
		sf::Event Event;
		while (App.GetEvent(Event))
		{
		...
		//event.mouseOver(_item,player,Event,MousePos,App);
		//event.hoverItem(_item, player, Event);
		
		//Draw Items	
		_item.displayItems(App,Event,player.player.ani_spr, MousePos,player);
		_item.displayStats(App,player);
		
		//Animation when moved
		player.player.Update(App, main, locked, player.stats.health, health);
